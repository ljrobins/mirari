...
@ti.func
def g_smith(O: ti.math.vec3, N: ti.math.vec3, L: ti.math.vec3, a2: float) -> float:
...
@ti.func
def fresnel_schlick(H: ti.math.vec3, L: ti.math.vec3, cs: float) -> float:
...
def sample_ggx_micro_normal_world(N: ti.math.vec3, a2: float) -> ti.math.vec3:
    ...
    y = ti.math.cross(N, x)
    dcm = ti.Matrix([[x[0], y[0], N[0]], [x[1], y[1], N[1]], [x[2], y[2], N[2]]])
