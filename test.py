...
def nearest_object(p: vec3):
    ...
    for i in ti.static(range(objects.shape[0])):
...
@ti.func
def raycast(ray: Ray, max_ray_steps: int):
...
def hemispheric_sampling(normal: vec3) -> vec3:
    return normalize(normal + mi.random_direction())


@ti.func
def raytrace(ray: Ray, max_ray_bounces: int, max_ray_steps: int) -> Ray:
    i = 0
    ti.loop_config(serialize=False)
    while i < max_ray_bounces:
        i += 1
        object, position, hit = raycast(ray, max_ray_steps)
        if not hit:
            ray.color = vec3(0)
            break
...
@ti.kernel
def render(camera_position: vec3, 
           camera_lookat: vec3, 
           camera_up: vec3, 
           fov: float, 
           max_ray_bounces: int,
    ...
    for i, j in image_pixels:
        ...
        half_width = half_height = tan(radians(fov) * 0.5)
...
def main():
    window = ti.ui.Window("Cornell Box", image_resolution, fps_limit=1000)
